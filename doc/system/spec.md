...
> **UI 設計**：組織架構各功能（地區/公司/事業部/項目/部門/職位/職級）整合在 `/system/org` 單一頁面，以 Tab 切換，不再作為獨立側欄選單項目。

### 3.1 地區管理（Tab: 地區）
**資料表：** `regions`
**欄位：** 地區名稱、地區代碼（如 TW / JP）、幣別、時區、**預設語系（defaultLocale）**、狀態
...
|------|---------------|------|
| TW（台灣） | zh-TW | 繁體中文 |
| JP（日本） | zh-TW | 日本區員工為華人，預設繁體中文 |
| UK（英國） | en | 英文 |

員工登入後，系統依其主要組織歸屬所屬公司的地區 `defaultLocale` 自動切換介面語系。使用者也可透過右上角的語系切換按鈕手動覆蓋。
...
### 3.7 職級管理（Tab: 職級）
**資料表：** `job_levels`
**欄位：** 職級代碼、職級名稱、排序（levelOrder，數字越大層級越高）、說明

**職級設計**（P/M 雙軌制）：

| 類別 | 代碼範圍 | levelOrder | 說明 |
|------|----------|------------|------|
| 一般員工 | P1 ~ P16 | 1 ~ 16 | 數字越大，資歷越深 |
| 管理崗 | M1 ~ M7 | 101 ~ 107 | 101 起，高於所有 P 等 |

> 已移除舊版 L1~L7 測試職級，正式資料使用 P/M 雙軌制。
...
| 語系代碼 | 名稱 | 備註 |
|----------|------|------|
| `zh-TW` | 繁體中文 | 預設語系，台灣區及日本區（華人員工） |
| `zh-CN` | 简体中文 | 可選 |
| `en`    | English | 英國區預設 |

### 語系切換機制

1. **登入自動設定**：員工登入後，系統取得主要組織歸屬的公司所屬地區 `defaultLocale`，自動切換介面語系
2. **手動切換**：右上角語系切換 Dropdown，選擇後即時生效並寫入 localStorage
3. **持久化**：語系偏好存於 localStorage（key: `oa_locale`），跨 session 保持

### 地區語系設定

在「系統管理 → 組織架構 → 地區」Tab 可設定每個地區的 `defaultLocale`。

### 技術實作

- 前端：`vue-i18n@9`（Composition API 模式）
- 語系檔案：`src/locales/zh-TW.ts`、`src/locales/zh-CN.ts`、`src/locales/en.ts`
- Element Plus：透過 `ElConfigProvider` 動態切換 Element Plus 元件語系
- 資料庫：`regions.defaultLocale VARCHAR(10) DEFAULT 'zh-TW'`
