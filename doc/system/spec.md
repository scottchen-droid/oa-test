...
系統管理員模塊提供整個 OA 系統的基礎設定，包含帳號管理、角色與權限設定、組織架構維護、審批流程設定及系統稽核。此模塊僅對 `isSuperAdmin = true` 或具備 `module.system.access` 的使用者開放。

**Route 前綴：** `/system`
**權限碼：** `module.system.access`（`isSuperAdmin` 無條件通過）
**可見對象：** `isSuperAdmin` 或具備 system 模塊角色的使用者
...
### 帳號列表
**篩選：** 帳號狀態（啟用/停用/鎖定）、是否超級管理員、有無員工資料
**欄位：** 工號（帳號）、顯示名稱、Email、帳號狀態、`isSuperAdmin`、最後登入時間、建立時間
