<template>
  <div>
    ...
    <!-- 載入中 -->
    <div v-if="groupsLoading" class="loading-placeholder">
    ...
    <el-empty
      ...
      description="您目前未加入任何工作單處理群組"
    />

    <template v-else>
      <!-- 群組 Tab -->
      <el-tabs v-model="activeGroupId" class="group-tabs" @tab-change="onGroupChange">
        <el-tab-pane
          v-for="g in groups"
          :key="g.id"
          :label="g.name"
...
const ui = useUiStore()
ui.setBreadcrumbs([{ title: '工作單中心' }, { title: '群組工作單' }])
...
async function loadGroups() {
  ...
  try {
    const all = await workOrderGroupsApi.adminList()
    // 顯示所有啟用群組（實際場景應呼叫 /api/work-order-groups/my，此處用 adminList 代替）
    groups.value = all.filter(g => g.isEnabled)
...
async function loadOrders() {
  ...
  loading.value = true
  try {
    const result = await workOrdersApi.getGroupOrders({
