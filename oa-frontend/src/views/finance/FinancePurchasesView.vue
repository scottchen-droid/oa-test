  ...
  <div>
    ...
    <el-card>
      ...
      <el-table v-loading="loading" :data="data" border stripe>
        ...
        <el-table-column prop="companyName" label="申請公司" width="140" />
        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template #default="{ row }">
            ...
            <template v-if="row.status === 'submitted'">
              <el-button text size="small" type="success">核准</el-button>
              <el-button text size="small" type="danger">駁回</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
...
import { purchaseRequestsApi } from '@/api/finance.api'
...
const { loading, data, total, pagination, fetch, handlePageChange, handleSizeChange } = useTable({
  fetchFn: (params) => purchaseRequestsApi.getAll(params as Parameters<typeof purchaseRequestsApi.getAll>[0]),
})

onMounted(() => {
  ui.setBreadcrumbs([{ title: '財務模塊' }, { title: '採購申請管理' }])
  fetch()
})
